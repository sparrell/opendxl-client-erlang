...
add_callback(Pid, Info) when is_record(Info, callback_info) ->
    gen_server:call(Pid, {add_callback, Info}).
...
handle_call({send_request, Topic, Message, Delay}, From, State) ->
    ...
    post(MQTTC, Topic, Message2, State),
    {noreply, State#state{pending_requests=PendingReqs1}, Delay};

handle_call({send_async_request, Topic, Message, Callback, Timeout}, From, State) ->
    #state{mqttc=MQTTC, callback_manager=CallbackManager, reply_to_topic=ReplyToTopic} = State,
    #dxlmessage{message_id=MessageId} = Message,
    Message1 = pre_process_message_out(request, Message, State),
    Message2 = Message1#{reply_to_topic=>ReplyToTopic},

    Filter = fun(_TopicIn, MessageIn) -> MessageIn#dxlmessage.message_id =:= MessageId end,
    Info = #callback_info{type=response, topic=ReplyToTopic, filter=Filter, callback=Callback, single_use=true, timeout=Timeout, args=[From, MessageId]},
    {ok, HandlerId} = gen_server:call(CallbackManager, {add_callback, Info}),
    post(MQTTC, Topic, Message2, State),
    {reply, ok, State};
...
handle_call({add_callback, Info}, _From, State) ->
    ...
    Result = gen_server:call(CallbackManager, {add_callback, Info}),
    {reply, Result, State};
...
handle_info({publish, Topic, Binary}, State) ->
    ...
    gen_server:cast(CallbackManager, {notify, Topic, Message, self()}),
    {noreply, State};
...
pre_process_message_out(Type, Message, State) when is_atom(Type) ->
    ...
    Message#dxlmessage{type=Type, message_id=MessageId, src_client_id=ClientId}.
